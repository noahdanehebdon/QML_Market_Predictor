...
def compare_to_naive_baselines(
    predictions, *, practical_margin=0.01, bootstrap_samples=2000, seed=42
):
    metrics = []
    for (model, split), frame in predictions.groupby(["model_name", "split_id"]):
        ...
        metrics.append(
            {"model_name": model, "split_id": split, "rank_ic": daily.mean()}
        )
    ...
    benchmark_rows = naive.loc[
        naive["model_name"].eq(benchmark), ["split_id", "rank_ic"]
    ].rename(columns={"rank_ic": "naive_ic"})
    rng, rows = np.random.default_rng(seed), []
    for model in sorted(set(metrics["model_name"]) - NAIVE_MODELS):
        paired = (
            metrics.loc[metrics["model_name"].eq(model), ["split_id", "rank_ic"]]
            .merge(benchmark_rows, on="split_id")
            .dropna()
        )
        delta = (paired["rank_ic"] - paired["naive_ic"]).to_numpy()
        if len(delta):
            samples = rng.choice(
                delta, size=(bootstrap_samples, len(delta)), replace=True
            ).mean(axis=1)
        ...
        rows.append(
            {
                "model_name": model,
                "naive_model": benchmark,
                "mean_rank_ic_margin": mean,
                "ci_lower": lower,
                "ci_upper": upper,
                "practical_margin": practical_margin,
                "beats_naive": bool(
                    pd.notna(mean) and mean >= practical_margin and lower > 0
                ),
            }
        )
