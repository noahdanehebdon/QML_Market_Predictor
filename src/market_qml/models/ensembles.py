...
def eligible_regime_weights(
    regimes: pd.DataFrame, *, regime_column: str, min_rows: int = 100
) -> pd.DataFrame:
    ...
    counts = (
        regimes.groupby(regime_column, dropna=True).size().rename("rows").reset_index()
    )
...
def build_chronological_ensembles(
    predictions: pd.DataFrame,
    *,
    min_history_rows=20,
    ...
    binary_models = {
        name
        for name, frame in base.groupby("model_name")
        if set(pd.to_numeric(frame["y_true"]).dropna().unique()) <= {0, 1}
    }
    all_predictions, diagnostics, sensitivities = [], [], []
    for task, models in [
        ("classification", binary_models),
        ("ranking", set(base["model_name"]) - binary_models),
    ]:
        if len(models) < 2:
            continue
        ...
        for split_id in sorted(task_data["split_id"].unique()):
            ...
            model_columns = [
                ...
                if m in current_wide and current_wide[m].notna().all()
            ]
            if len(model_columns) < 2:
                continue
            calibrated_current = current_wide[model_columns].copy()
            calibrated_history = history_wide.reindex(
                columns=history_wide.columns.union(model_columns)
            )
            ...
            methods = {
                "simple_average_ensemble": calibrated_current[model_columns].mean(
                    axis=1
                ),
                "rank_average_ensemble": calibrated_current.groupby(
                    current_wide["date"]
                )[model_columns]
                .rank(pct=True)
                .mean(axis=1),
                "constrained_stack_ensemble": calibrated_current[
                    model_columns
                ].to_numpy()
                @ learned,
            }
    ...
    return EnsembleResult(
        combined, pd.DataFrame(diagnostics), pd.DataFrame(sensitivities)
    )
...
def _calibrate(history, current, model, minimum):
    ...
    if len(valid) < minimum or valid["y_true"].nunique() < 2:
        return current[model].clip(1e-6, 1 - 1e-6), history.get(
            model, pd.Series(index=history.index, dtype=float)
        ).clip(1e-6, 1 - 1e-6)
    calibrator = LogisticRegression(random_state=42).fit(
        valid[[model]], valid["y_true"].astype(int)
    )
    ...
    history_scores.loc[mask] = calibrator.predict_proba(history.loc[mask, [model]])[
        :, 1
    ]
...
def _learn_weights(
    history, models, task, minimum, turnover_penalty, instability_penalty
):
    ...
    def objective(weights):
        ...
        if task == "classification":
            base = log_loss(
                y.astype(int), np.clip(score, 1e-6, 1 - 1e-6), labels=[0, 1]
            )
            fold_values = (
                usable.assign(_score=score)
                .groupby("split_id")
                .apply(
                    lambda f: log_loss(
                        f["y_true"].astype(int),
                        np.clip(f["_score"], 1e-6, 1 - 1e-6),
                        labels=[0, 1],
                    ),
        ...
        return (
            base
            + instability_penalty * float(fold_values.std(ddof=0))
            + turnover_penalty
            * float(turnover_proxy if pd.notna(turnover_proxy) else 0)
        )
    ...
    return (result.x if result.success else equal), (
        "chronological_constrained"
        if result.success
        else "equal_weight_optimizer_failure"
    )
