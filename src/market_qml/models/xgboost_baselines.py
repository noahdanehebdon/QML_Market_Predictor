...
def train_xgboost_classifier(
    ...
    *,
    ...
    model.fit(
        data.train.X.loc[inner_train],
        data.train.y.loc[inner_train].astype(int),
        sample_weight=weights,
        eval_set=[
            (data.train.X.loc[inner_valid], data.train.y.loc[inner_valid].astype(int))
        ],
        verbose=False,
    )
        ...
        [
            {
                ...
                "objective": "logloss",
                "best_iteration": best,
                "inner_validation_logloss": float(model.best_score),
            }
        ]
    )
    return XGBoostResult(
        model,
        predictions,
        {"model": model_name, "best_iteration": best, "date_aware_weights": True},
        diagnostics,
    )
...
def train_xgboost_ranker(
    ...
    *,
    ...
    train = _rank_frame(
        data.train.X.loc[inner_train],
        data.train.y.loc[inner_train],
        data.train.metadata.loc[inner_train],
    )
    valid = _rank_frame(
        data.train.X.loc[inner_valid],
        data.train.y.loc[inner_valid],
        data.train.metadata.loc[inner_valid],
    )
    model = XGBRanker(
        n_estimators=500,
        max_depth=4,
        ...
        [
            {
                ...
                "objective": "rank:ndcg",
                "best_iteration": best,
                "inner_validation_ndcg": float(model.best_score),
            }
        ]
    )
    return XGBoostResult(
        model,
        predictions,
        {"model": model_name, "best_iteration": best, "grouped_by": "date"},
        diagnostics,
    )
...
def _rank_frame(X, y, metadata):
    ...
    relevance = (
        ordered_y.groupby(pd.to_datetime(metadata.loc[order, "date"]))
        .rank(pct=True)
        .mul(4)
        .round()
        .astype(int)
