...
def analyze_predictions_by_regime(
    ...
    *,
    ...
    predictions["date"] = pd.to_datetime(
        predictions["date"], errors="coerce"
    ).dt.normalize()
    regime_table = regimes[["date", *REGIME_COLUMNS]].copy()
    regime_table["date"] = pd.to_datetime(
        regime_table["date"], errors="coerce"
    ).dt.normalize()
    if regime_table["date"].duplicated().any():
        raise ValueError("Regime table must contain one row per date")
    joined = predictions.merge(
        regime_table, on="date", how="left", validate="many_to_one"
    )

    rows = []
    for regime_type in REGIME_COLUMNS:
        available = joined.dropna(subset=[regime_type])
        for (regime, model), group in available.groupby(
            [regime_type, "model_name"], sort=True
        ):
            rows.append(
                _metric_row(group, regime_type, str(regime), str(model), minimum_rows)
            )
...
def save_regime_analysis(
    result: RegimeAnalysisResult, output_dir: str | Path
) -> dict[str, Path]:
...
def render_regime_report(result: RegimeAnalysisResult) -> str:
    ...
    for regime_type in REGIME_COLUMNS:
        ...
        for regime, slice_metrics in group.groupby("regime", sort=True):
            roc = slice_metrics.dropna(subset=["roc_auc"]).sort_values(
                "roc_auc", ascending=False
            )
            rank = slice_metrics.dropna(
                subset=["rank_information_coefficient"]
            ).sort_values("rank_information_coefficient", ascending=False)
            if not roc.empty:
                lines.append(
                    f"- **{regime}:** ROC-AUC leader is `{roc.iloc[0].model_name}` ({roc.iloc[0].roc_auc:.4f})."
                )
            if not rank.empty:
                lines.append(
                    f"  Rank-IC leader is `{rank.iloc[0].model_name}` ({rank.iloc[0].rank_information_coefficient:.4f})."
                )
        lines.append("")
    ...
    if not qcnn.empty:
        ...
        if not best.empty:
            lines.append(
                f"QCNN's strongest ROC-AUC slice is **{best.iloc[0].regime}** ({best.iloc[0].roc_auc:.4f}); its weakest is **{worst.iloc[0].regime}** ({worst.iloc[0].roc_auc:.4f})."
            )
        lines.extend(
            [
                "",
                "Treat regime gaps as descriptive unless they persist across multiple chronological splits and adequate sample sizes.",
                "",
            ]
        )
    return "\n".join(lines)
...
def _metric_row(group, regime_type, regime, model_name, minimum_rows):
    ...
    rank_ic = (
        score.corr(returns, method="spearman")
        if enough and score.nunique() > 1 and returns.nunique() > 1
        else np.nan
    )
    top_count = max(1, int(np.ceil(len(group) * 0.1)))
    top_return = (
        group.assign(_score=score.to_numpy())
        .nlargest(top_count, "_score")["forward_excess_return"]
        .mean()
    )
    return {
        ...
        "rows": len(group),
        "splits": group["split_id"].nunique(),
        "positive_rate": y.mean(),
        "meets_minimum_rows": enough,
...
def _qcnn_differences(metrics):
    ...
    merged = qcnn.merge(
        others, on=["regime_type", "regime"], suffixes=("_qcnn", "_comparison")
    )
    for metric in [
        "roc_auc",
        "accuracy",
        ...
        merged[f"{metric}_difference"] = (
            merged[f"{metric}_qcnn"] - merged[f"{metric}_comparison"]
        )
    return merged
...
def _validate_inputs(predictions, regimes, minimum_rows):
    ...
    if missing_predictions:
        raise ValueError(
            "Predictions are missing columns: " + ", ".join(sorted(missing_predictions))
        )
    missing_regimes = {"date", *REGIME_COLUMNS} - set(regimes.columns)
    if missing_regimes:
        raise ValueError(
            "Regimes are missing columns: " + ", ".join(sorted(missing_regimes))
        )
