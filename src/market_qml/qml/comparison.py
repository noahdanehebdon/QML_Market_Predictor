...
class ComparisonConfig:
    ...
    feature_selection_names: tuple[str, ...] = tuple(DEFAULT_FEATURE_SELECTIONS)
...
def run_model_comparison(data: pd.DataFrame, config: ComparisonConfig = ComparisonConfig()) -> ComparisonResult:
    ...
    for split_id in sorted(data["split_id"].unique()):
        ...
        primary = build_qml_train_validation(
            sampled, split_id=int(split_id), feature_columns=DEFAULT_FEATURE_SELECTIONS["broad_market"]
        )
        ...
        runners = {
            ...
            "qcnn": lambda: train_qcnn(primary, max_iter=config.qcnn_iterations,
                                        ...
                                        random_state=config.random_state + int(split_id)).predictions,
            "qsvm": lambda: _qsvm_predictions(primary, 1.0, 2, "qsvm", config.random_state),
        ...
        tuned_data = build_qml_train_validation(
            sampled, split_id=int(split_id), feature_columns=DEFAULT_FEATURE_SELECTIONS[chosen["feature_selection"]]
        )
        runners["qsvm_tuned"] = lambda d=tuned_data, c=chosen: _qsvm_predictions(
            d, float(c["C"]), int(c["repetitions"]), "qsvm_tuned", config.random_state
        )
...
def _validate_inputs(data, config):
    ...
    required.update(c for name in config.feature_selection_names for c in DEFAULT_FEATURE_SELECTIONS[name])
