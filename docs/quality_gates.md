...
Feature-level evidence is generated by the separate `Feature research smoke` workflow.
The frozen model evaluation does not recompute the quadratic diagnostic audit inline;
this separation changes no training rows, targets, models, inference, or promotion
thresholds.
