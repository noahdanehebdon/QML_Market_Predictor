...
def test_complex_model_requires_practical_and_statistical_margin():
    ...
    for split in range(4):
        for model, scores in [
            ("momentum_rank", [0, 1, 2, 3]),
            ("xgboost_ranker", [0, 1, 2, 3]),
        ]:
                ...
                rows.append(
                    {
                        ...
                        "y_score": score,
                        "forward_excess_return": index
                        if model == "xgboost_ranker"
                        else [1, 0, 3, 2][index],
                    }
                )
    result = compare_to_naive_baselines(
        pd.DataFrame(rows), practical_margin=0.01, bootstrap_samples=100
    )
