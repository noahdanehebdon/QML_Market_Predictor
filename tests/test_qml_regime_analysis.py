...
from market_qml.qml.regime_analysis import (
    analyze_predictions_by_regime,
...
def _predictions():
    ...
    for model_index, model in enumerate(["qcnn", "vqc", "qsvm", "logistic_regression"]):
        for index, date in enumerate(dates):
            target = index % 2
            score = (
                (0.8 if target else 0.2)
                if model == "qcnn"
                else 0.5 + model_index * 0.01
            )
            rows.append(
                {
                    "symbol": f"S{index}",
                    ...
                    "forward_excess_return": 0.01 if target else -0.02,
                }
            )
...
def test_regime_analysis_slices_models_and_compares_qcnn():
    ...
    assert set(result.metrics.regime_type) == {
        "volatility_regime",
        "rate_regime",
        "yield_curve_regime",
    }
    ...
    low_qcnn = result.metrics.query(
        "regime == 'low_volatility' and model_name == 'qcnn'"
    ).iloc[0]
...
def test_missing_regime_dates_remain_explicitly_unlabeled():
    result = analyze_predictions_by_regime(
        _predictions(), _regimes().iloc[:-1], minimum_rows=2
    )
