...
def test_build_macro_daily_normalizes_mixed_timestamp_resolutions(tmp_path):
    price_path = tmp_path / "prices.parquet"
    macro_path = tmp_path / "macro.parquet"
    output_path = tmp_path / "macro_daily.parquet"
    ...
    prices = pd.DataFrame(
        {
            "date": pd.Series(["2024-02-01", "2024-02-02"], dtype="datetime64[s]"),
            "symbol": ["SPY", "SPY"],
            "close": [100.0, 101.0],
        }
    )
    ...
    macro = pd.DataFrame(
        ...
        index=pd.DatetimeIndex(
            pd.Series(["2024-01-01"], dtype="datetime64[us]"), name="date"
        ),
    )
    macro.to_parquet(macro_path)
    ...
    assert result.loc[pd.Timestamp("2024-02-01"), "treasury_10y"] == 4.0
