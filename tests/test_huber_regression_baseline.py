...
def test_train_huber_regression_outputs_standard_prediction_table():
    result = train_huber_regression(_preprocessed())

    assert isinstance(result.model, HuberRegressor)
...
def test_train_huber_regression_rejects_missing_training_targets():
    data = _preprocessed()
...
def test_huber_regression_outputs_can_be_saved(tmp_path):
    result = train_huber_regression(_preprocessed())
    model_path = tmp_path / "model.pkl"
    prediction_path = tmp_path / "predictions.parquet"

    save_huber_regression_model(result.model, model_path)
