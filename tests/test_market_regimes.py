...
def _features(rows=10):
    ...
    spy_returns = [0.01, -0.01, 0.02, -0.02, 0.03, -0.03, 0.01, -0.01, 0.04, -0.04][:rows]
    frames = []
...
def test_build_market_regimes_defines_volatility_rates_and_curve():
    result = build_market_regimes(
        _features(), volatility_window=3, rate_window=2,
        annualization_factor=1, minimum_threshold_history=2,
    )
    assert result.date.is_unique
    assert result.loc[4, "volatility_regime"] in {"high_volatility", "low_volatility", "normal_volatility"}
...
def test_regimes_do_not_change_when_future_data_changes():
    ...
    kwargs = dict(volatility_window=3, rate_window=2, annualization_factor=1,
...
def test_volatility_threshold_excludes_current_observation():
    ...
    baseline = build_market_regimes(
        features, volatility_window=3, rate_window=2,
        annualization_factor=1, minimum_threshold_history=2,
    )
    changed = features.copy()
    changed.loc[(changed.symbol == "SPY") & (changed.date == "2024-01-07"), "return_1d"] = 5.0
    stressed = build_market_regimes(
        changed, volatility_window=3, rate_window=2,
        annualization_factor=1, minimum_threshold_history=2,
    )
    assert stressed.loc[6, "spy_realized_volatility"] != baseline.loc[6, "spy_realized_volatility"]
    assert stressed.loc[6, "spy_volatility_threshold"] == baseline.loc[6, "spy_volatility_threshold"]
