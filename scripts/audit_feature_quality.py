...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("reports/feature_audit"))
...
def main() -> None:
    ...
    result = audit_features(
        pd.read_parquet(args.features), pd.read_parquet(args.labels), pd.read_parquet(args.splits),
    ...
    manifest = {"features": int(result.stability.shape[0]), "splits": int(result.quality["split_id"].nunique()), "stable_features": int(result.stability.get("stable_evidence", pd.Series(dtype=bool)).sum()), "artifacts": sorted(f"{name}.parquet" for name in tables)}
