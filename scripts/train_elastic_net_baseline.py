"""Train ElasticNet regression baseline on one walk-forward split."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train ElasticNet regression baseline on a walk-forward split."
    )
    parser.add_argument(
        "--features",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0001,
        help="ElasticNet regularization strength.",
    )
    parser.add_argument(
        "--l1-ratio",
        type=float,
        default=0.5,
        help="Mixing parameter between L1 and L2 penalties.",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Random seed for reproducible model fitting.",
    )
    parser.add_argument(
        "--predictions-output",
        type=Path,
...
def main() -> None:
    ...
    save_elastic_net_model(result.model, args.model_output)
