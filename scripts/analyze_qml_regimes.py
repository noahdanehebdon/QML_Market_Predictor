...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze model performance by market regime."
    )
    parser.add_argument(
        "--predictions",
        type=Path,
        default=Path("reports/qml_comparison/predictions.parquet"),
    )
    parser.add_argument(
        "--regimes", type=Path, default=Path("data/features/market_regimes.parquet")
    )
